<script lang="ts">
	...
	let { project, warnings, isConflict, alsoIn, loader }: Props = $props();
		...
		<Card.Content class="flex gap-3 p-3">
			...
			<div class="min-w-0 flex-1">
				...
				<div class="flex items-start justify-between gap-2">
					<a
						href="https://modrinth.com/{project.projectType}/{project.projectSlug}"
						target="_blank"
				...
				{#if project.projectDescription}
					<p class="line-clamp-1 text-sm text-muted-foreground">{project.projectDescription}</p>
				{:else}
				...
				<div class="mt-1.5 flex flex-wrap items-center gap-1">
					{#each project.loaders.filter((l) => l === loader || project.usedFallbackLoader) as loaderName (loaderName)}
						<Badge
							variant="secondary"
							class="text-[10px] leading-tight {getBadgeClassesByModLoader(loaderName)}"
						>
							{loaderName}
						</Badge>
					{/each}
					...
					{#if project.versionType !== 'release'}
						<Badge
							variant="secondary"
							class="text-[10px] leading-tight {VERSION_TYPE_BADGE_CLASSES[project.versionType] ??
								''}"
						>
							{project.versionType}
						</Badge>
					...
					{#if project.usedFallbackLoader && project.resolvedLoader}
						<Badge variant="outline" class="text-[10px] leading-tight">
							via {project.resolvedLoader}
						</Badge>
					{/if}

					<Tooltip.Root>
						<Tooltip.Trigger class="ml-auto">
							<svelte:component this={TypeIcon} class="size-3.5 text-muted-foreground" />
						</Tooltip.Trigger>
