<script lang="ts">
    ...
    import ModCardExpanded from './ModCardExpanded.svelte';
    import { getBadgeClassesByModLoader, VERSION_TYPE_BADGE_CLASSES } from '$lib/utils/colors';
    import {
        formatBytes,
        formatNumber,
        formatRelativeTime,
    ...
    let hasTier3 = $derived(
        !!(project.changelog?.trim() || project.gallery?.length || project.fileHashes)
    );
</script>
        ...
        >
            <Card.Content class="flex gap-3 p-3">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    {:else}
                        <p class="line-clamp-1 text-sm text-muted-foreground italic">
                            No description
                        </p>
                    {/if}
                    ...
                    <div class="mt-1.5 flex flex-wrap items-center gap-1">
                        {#each project.loaders.filter((l) => l === loader || project.usedFallbackLoader) as loaderName (loaderName)}
                            <Badge
                                ...
                                class={cn(
                                    ...
                                    getBadgeClassesByModLoader(loaderName)
                                )}
                            >
                        ...
                        <Badge
                            ...
                            class={cn(
                                'text-[10px] leading-tight',
                                SIDE_BADGE_CLASSES[project.side]
                            )}
                        >
                            {sideLabel}
                        </Badge>
                    ...
                    <!-- Tier 2 detail row -->
                    {#if project.categories?.length || project.downloadCount || project.licenseName}
                        <div
                            class="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground"
                        >
                            {#each (project.categories ?? []).slice(0, 3) as cat (cat)}
                                <Badge variant="outline" class="text-[9px]">{cat}</Badge>
                            {/each}
                            {#if project.downloadCount}
                                <span>{formatNumber(project.downloadCount)} downloads</span>
                            {/if}
                            {#if project.licenseName}
                    ...
                    <div class="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        {#if hasTier3}
                            <Collapsible.Trigger
                                class="ml-auto flex items-center gap-0.5 text-primary hover:underline"
                            >
